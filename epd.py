...
class PiAware:
    ...
    class Helpers:
        ...
        def send_slack_notification(
            ...
            if slack_token is not None and slack_channel is not None:
                PiAware.Helpers.check_supported(mode, ['emergency', 'registration', 'icao'])
                ...
                try:
                    ...
                    if mode == 'emergency':
                        ...
                        header_block = f'Registration of Special Interest on {PIAWARE_HOST}'
                    ...
                    message_blocks = [
                        ...
                        slack_sdk.models.blocks.blocks.SectionBlock(text=description_block),
                        slack_sdk.models.blocks.blocks.DividerBlock(),
                        slack_sdk.models.blocks.blocks.SectionBlock(fields=[
                            PiAware.Helpers.format_slack_field('Aircraft', icao_reg),
                            PiAware.Helpers.format_slack_field('Callsign', callsign),
                            PiAware.Helpers.format_slack_field('Squawk', squawk),
                            PiAware.Helpers.format_slack_field('Distance', distance_field),
    ...
    def __process_special_interest(aircraft: list) -> None:
        ...
        if 'flight' in aircraft and len(REGISTRATION_OF_SPECIAL_INTEREST) >= 1:
            ...
            contains_registration = PiAware.Helpers.contains_any(
                flight,
                list(REGISTRATION_OF_SPECIAL_INTEREST.keys())
            )
            ...
            if contains_registration is True:
                ...
                if sent is True:
                    logging.info(
                        ...
                        flight, distance, REGISTRATION_OF_SPECIAL_INTEREST[flight]
                    )
        ...
        if 'hex' in aircraft and len(ICAO_OF_SPECIAL_INTEREST) >= 1:
            ...
            contains_icao = PiAware.Helpers.contains_any(
                hex,
                list(ICAO_OF_SPECIAL_INTEREST.keys())
            )
            ...
            if contains_icao is True:
                ...
                if sent is True:
                    logging.info(
                        ...
                        hex.lower(), distance, ICAO_OF_SPECIAL_INTEREST[hex.upper()]
                    )
                ...
                if 'squawk' in aircraft:
                    ...
                    if contains:
                        ...
                        logging.warning(
                            ...
                            aircraft['hex'], flight, aircraft['squawk'], aircraft_distance, notification_sent
                        )
        else:
            emergency = 666
    ...
    def refresh(cycle: int = 1) -> int:
        ...
        try:
            ...
            logging.debug('e-Paper Display is %sx%s (Height x Width)', epd.height, epd.width)
            ...
            logging.info('Refresh cycle %s complete after %ss', cycle, round(time.time() - time_start, 3))
    ...
    def process() -> None:
        ...
        while True:
            ...
            logging.info('Sleeping for %s seconds after refresh cycle %s (Ping every %ss)', sleepy_display, current_cycle, sleepy_updates)
            while i < sleepy_display:
                time.sleep(1)
                i += 1
                if i % sleepy_updates == 0:
                    logging.info('Slept %s/%s seconds after refresh cycle %s', i, sleepy_display, current_cycle)
    ...
    def setup() -> None:
        ...
        if SENTRY_DSN is not None and PiAware.Helpers.is_valid_url(SENTRY_DSN):
            ...
            logging.info('Skipping Sentry SDK Configuration. SENTRY_DSN is not set or invalid.')
        ...
        GPIO.add_event_detect(5, GPIO.FALLING, callback=PiAware.__process_interrupt, bouncetime=200)
        GPIO.add_event_detect(6, GPIO.FALLING, callback=PiAware.__process_interrupt, bouncetime=200)
        GPIO.add_event_detect(13, GPIO.FALLING, callback=PiAware.__process_interrupt, bouncetime=200)
        GPIO.add_event_detect(19, GPIO.FALLING, callback=PiAware.__process_interrupt, bouncetime=200)
